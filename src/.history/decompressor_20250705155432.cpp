...
static T read_u(std::istream& is){
    T v=0;  for(size_t i=0;i<sizeof(T);++i){ int b=is.get();
        if(b==EOF) throw std::runtime_error("EOF"); v|=uint64_t(uint8_t(b))<<(8*i); }
    return v;
}
                     ...
                     const std::string& out_path)
{
    ...
    uint8_t marker = in.get();

    if (marker == 0x01){
        uint64_t n = read_u<uint64_t>(in);
        char     c = in.get();
        std::vector<char> buf(4096,c);
        while(n){ size_t k=std::min<uint64_t>(n,buf.size());
                   out.write(buf.data(),k); n-=k; }
        return;
    }
    if (marker != 0x00) throw std::runtime_error("bad marker");
    ...
    if (emitted != original) throw std::runtime_error("truncated bit-stream");
}
